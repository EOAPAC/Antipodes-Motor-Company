...
Four articles at `/journal/`, generated by `web/build-pages.py` so the copy
lives in one file and the template cannot drift between pages. The same script
builds `/commission/` from `web/pages_commission.py`, which is why the shell,
the nav and the enquiry form are identical on every generated page.
